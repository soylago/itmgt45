...
def tic_tac_toe(board):
    ...
    for i in range(len(board)):
        ...
        if all(cell == board[0][i] and cell != '' for cell in [board[j][i] for j in range(len(board))]):
            return board[0][i]

    
    if all(cell == board[0][0] and cell != '' for cell in [board[i][i] for i in range(len(board))]):
        return board[0][0]
    if all(cell == board[0][len(board) - 1] and cell != '' for cell in [board[i][len(board) - 1 - i] for i in range(len(board))]):
...
def eta(first_stop, second_stop, route_map):
    ...
    available_keys=list(legs.keys())
    firststopkey= [item[0]for item in available_keys]
    secondstopkey=[item[1]for item in available_keys]
    accutime=list(legs.values())
    starttime=[]
    finaltime=[]
    
   
    firstposition=firststopkey.index(first_stop)
    secondposition=secondstopkey.index(second_stop)
      
    for x in range(len(accutime)):
            starttime=starttime+list(accutime[x].values())
    
    if first_stop==second_stop:
        return sum(starttime)
    elif firstposition==secondposition:
        return starttime[firstposition]
    elif firstposition<secondposition:
        finaltime=starttime[firstposition:secondposition+1]
        return sum(finaltime)
    elif firstposition>secondposition:
        finaltime=starttime[firstposition:]+starttime[:secondposition+1]
        return sum(finaltime)